...
# ═══════════════════════════════════════════════════
# ConjSessionManager — 变形条目练习引擎
# 与 gist 的 SessionManager 逻辑对齐：
#   状态 3=初见/不会  2=模糊  1=通过
#   组内循环 + 跨组携带 + 累计失败追踪
# ═══════════════════════════════════════════════════
_CONJ_REAPPEAR   = {3:[('near',3,6),('medium',12,20)], 2:[('medium',8,14)], 1:[]}
_CONJ_CARRY      = {3:2, 2:1}   # 跨组时插入次数
CONJ_FAIL_THRESH = 3            # 累计③次数达此值 → 卡词，强制跨组

class ConjSessionManager:
    def __init__(self, items, ordered=False):
        """
        items: list of practice dicts, each must have 'group' key (int)
        ordered: if True, don't shuffle within group
        """
        self.items   = items
        self.ordered = ordered
        # key per item: word::form_id (unique enough for display)
        self.keys    = [f"{it['word']}::{it['form_id']}" for it in items]
        self.idx_map = {k: i for i, k in enumerate(self.keys)}

        self.state    = {k: 3 for k in self.keys}
        self.done     = {k: False for k in self.keys}
        self.dgr      = {k: {} for k in self.keys}   # short-term upgrade counter
        self.fail_cnt = {k: 0 for k in self.keys}    # cumulative ③ count
        self.history  = {k: [] for k in self.keys}

        # group structure
        gmap = {}
        for it in items:
            gmap.setdefault(it['group'], []).append(f"{it['word']}::{it['form_id']}")
        self.group_order = sorted(gmap.keys())
        self.gmap        = gmap
        self.g_idx       = 0
        self.carryover   = {}        # {key: slots}
        self.last_carryover = []
        self.queue       = []
        self.q_pos       = 0
        self.in_loop     = False
    ...
    def _build_queue(self):
        ...
        main = [k for k in self.gmap[gid] if not self.done[k]]
        if not self.ordered:
            random.shuffle(main)
        combined = list(main)
        for k, slots in self.carryover.items():
            for _ in range(slots):
                combined.insert(random.randint(0, len(combined)), k)
        self.carryover = {}
        self.queue = combined
        self.q_pos = 0
    ...
    def current(self):
        if self.q_pos >= len(self.queue):
            return None, -1
        k   = self.queue[self.q_pos]
        idx = self.idx_map.get(k, -1)
        return (self.items[idx] if idx >= 0 else None), idx

    def _reinsert(self, key):
        for (_, lo, hi) in _CONJ_REAPPEAR.get(self.state[key], []):
            dist = max(2, random.randint(lo, hi))
            pos  = min(self.q_pos + dist, len(self.queue))
            self.queue.insert(pos, key)

    def rate(self, rating):
        """rating: 1=听懂  2=不确定  3=没听懂"""
        item, idx = self.current()
        if idx < 0: return 'done'
        k = self.keys[idx]
        self.history[k].append(rating)
        ...
        if rating < curr:
            ...
            self.dgr[k][rating] = self.dgr[k].get(rating, 0) + 1
            if self.dgr[k][rating] >= 3:
                self.state[k] = min(3, curr + 1)
                self.dgr[k]   = {}
        else:
            self.dgr[k] = {}
        ...
        self.q_pos += 1
        if self.is_done(): return 'session_done'
        if self.q_pos >= len(self.queue):
    ...
    def _advance(self):
        if self.in_loop:
            undone = [k for k, d in self.done.items() if not d]
            if undone:
                if not self.ordered: random.shuffle(undone)
                self.queue = undone; self.q_pos = 0
                return 'continue'
            return 'session_done'
        ...
        new_carry = {}
        self.last_carryover = []
        for k in self.gmap[gid]:
            sv = self.state[k]
            if not self.done[k] and (sv in _CONJ_CARRY or
                                     self.fail_cnt.get(k, 0) >= CONJ_FAIL_THRESH):
                new_carry[k] = _CONJ_CARRY.get(sv, 1)
                self.last_carryover.append(k)
    ...
    def skip(self):
        ...
        self.q_pos += 1
        if self.is_done(): return 'session_done'
        if self.q_pos >= len(self.queue): return self._advance()
        return 'continue'

    def current_gid(self):
        if self.in_loop or self.g_idx >= len(self.group_order): return None
        return self.group_order[self.g_idx]

    def item_detail(self, key):
        return {
            'state':    self.state.get(key, 3),
            'dgr_cnt':  sum(self.dgr.get(key, {}).values()),
            'fail_cnt': self.fail_cnt.get(key, 0),
    ...
    def stats(self):
        ...
        total = len(self.done)
        stuck = sum(1 for k in self.fail_cnt
                    if self.fail_cnt[k] >= CONJ_FAIL_THRESH)
        return {
            'done':      done,
            'total':     total,
    ...
    def stuck_items(self):
        return [k for k in self.fail_cnt if self.fail_cnt[k] >= CONJ_FAIL_THRESH]
...
def _init():
    defaults={
        ...
        "batch_size":30,"show_group_done":False,
        "group_done_carry":[],"group_done_gid":"?",
    }
...
def screen_main():
    ...
    with c_info:
        st.caption("全量" if fpw >= max_forms else f"{fpw}/{max_forms}")

    # ③ 练习模式
    mode = st.radio("练习模式", ["🎧 普通（词库顺序）", "🔀 相似音（按读音排序）"],
                    horizontal=True, label_visibility="collapsed")

    # ④ 每组条目数
    st.caption("**每组条目数**（按此大小分批；卡词自动跨组继续出现）")
    all_gids = list({w["group"] for w in store.words})
    ws_all   = store.filter(lv_sel, all_gids)
    raw_items = cs.build_practice_items(ws_all, ev, ea, fpw_actual)
    if "相似音" in mode:
        raw_items = sort_by_similarity([it for it in raw_items if it.get("conj_reading")])
    n_raw = len(raw_items)

    bs = st.number_input("每组条目数", min_value=5, max_value=500,
                         value=st.session_state.get("batch_size", 30), step=5,
                         label_visibility="collapsed", key="batch_size_inp")
    ...
    for i, it in enumerate(raw_items):
        gid = (i // bs) + 1
        tmp_groups.setdefault(gid, []).append(it)

    def _grp_label(gid):
        its   = tmp_groups[gid]
        words = list(dict.fromkeys(it["word"] for it in its))   # unique, order-preserving
        prev  = "　".join(words[:4]) + ("…" if len(words) > 4 else "")
        return f"第{gid}组（{len(its)}条）：{prev}"

    all_tmp = list(tmp_groups.keys())
    gr_sel  = st.multiselect("选择本次练习的组", all_tmp, default=all_tmp,
    ...
    sel_items = [it for gid in sorted(gr_sel) for it in tmp_groups[gid]]
    # 重新编连续组号
    batched   = [{**it, 'group': (i // bs) + 1} for i, it in enumerate(sel_items)]

    st.info(f"🎯 **{len(sel_items)}** 条目  ·  **{len(gr_sel)}** 组  ·  来自 **{len({it['word'] for it in sel_items})}** 词")

    voice_name = st.selectbox("语音", list(VOICES.keys()))

    if st.button("▶ 开始练习", type="primary", use_container_width=True,
                 disabled=not batched):
        st.session_state.voice      = VOICES[voice_name]
        st.session_state.session    = ConjSessionManager(
...
# ═══════════════════════════════════════════════════
# 相似音排序工具（聚类后排序，喂给普通 PracticeSession）
# ═══════════════════════════════════════════════════
def sort_by_similarity(items):
    """按 (form_id, 变形完整读音) 排序，同一变形类型下读音相近的词自然相邻。"""
    def _key(it):
        r = (it.get("conj_reading") or it.get("conjugated","")).strip()
        return (it.get("form_id",""), r, it.get("word",""))
...
def screen_session():
    ...
    # 组切换提示
    if st.session_state.get("show_group_done"):
        carry = st.session_state.get("group_done_carry", [])
        gid   = st.session_state.get("group_done_gid", "?")
        if carry:
            names = "、".join(k.split("::")[0] for k in carry[:6])
            suffix = f"…等 {len(carry)} 条" if len(carry) > 6 else ""
            st.info(f"✅ 第 {gid} 组结束 · 携带 **{len(carry)}** 条卡词进入下一组：{names}{suffix}")
        else:
            st.success(f"✅ 第 {gid} 组全部通过！进入下一组")
        if st.button("继续 ▶", type="primary", use_container_width=True):
            st.session_state.show_group_done = False
            st.rerun()
        return
    ...
    key = f"{item['word']}::{item['form_id']}"
    det = sess.item_detail(key)
    s   = sess.stats()

    # ── 状态栏 ──
    _sl  = {1:"✅ 通过", 2:"🟡 模糊", 3:"❌ 不会"}
    gstr = f"第 {s['gid']} 组" if s['gid'] else "🔁 收尾"
    dgr_tip  = f" · 升难 {det['dgr_cnt']}/3" if det['dgr_cnt'] else ""
    fail_tip = ""
    if det['fail_cnt'] >= CONJ_FAIL_THRESH:
        fail_tip = f" · ⚠️ 卡词(③×{det['fail_cnt']})"
    elif det['fail_cnt'] > 0:
    ...
    st.caption(
        ...
        f"通过 {s['done']}/{s['total']}"
        + (f" · 卡 {s['stuck']}" if s['stuck'] else "")
    )
    st.progress(s['done'] / max(s['total'], 1))
    st.caption(f"{_sl[det['state']]}{dgr_tip}{fail_tip}")
    ...
    def do_rate(lv):
        ...
        if r == 'session_done':
            ...
            st.session_state.show_group_done  = True
            st.session_state.group_done_carry = list(sess.last_carryover)
            prev_gid = sess.group_order[sess.g_idx - 1] if sess.g_idx > 0 else "?"
            st.session_state.group_done_gid   = prev_gid
        st.rerun()
    ...
    with n2:
        if st.button("跳过 ▶", use_container_width=True):
            r = sess.skip()
            st.session_state.last_audio_key = ""
            if r == 'session_done':   st.session_state.phase = "done"
            elif r == 'group_done':
                st.session_state.show_group_done  = True
                st.session_state.group_done_carry = list(sess.last_carryover)
                prev_gid = sess.group_order[sess.g_idx-1] if sess.g_idx > 0 else "?"
    ...
    with st.expander(f"长期标记（当前 {cur_lv} 级）"):
        ...
        for lv, col in zip([0,1,2,3], cols):
            ...
            if col.button(lv_names[lv], key=f"ll_{lv}",
                          use_container_width=True, type=bt):
