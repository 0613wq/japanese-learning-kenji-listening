...
def screen_session():
    ...
    if st.session_state.cur_audio:
        st.audio(st.session_state.cur_audio,
                 format="audio/mp3", autoplay=autoplay)
    ...
    with nc1:
        if st.button("◀ 上一", disabled=(sess.q_pos == 0),
            ...
            if sess.prev():
                ...
                st.rerun()
    with nc2:
        # key='show_word' 让 toggle 直接读写 session_state.show_word
        st.toggle("👁 显词", key='show_word')
    with nc3:
