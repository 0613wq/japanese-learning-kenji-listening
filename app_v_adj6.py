...
# 相似音辨别练习  —  工具 & 会话
# ═══════════════════════════════════════════════════
def _sim_key(reading, n=2):
    """取读音末尾 n 个字符作为相似分桶 key"""
    r=(reading or "").strip()
    return r[-n:] if len(r)>=n else r

def build_similarity_pool(items):
    """
    按变形读音末尾2字分桶，返回 {key: [item,...]}
    只保留含 ≥2 个不同原词的桶（有辨别意义）
    """
    from collections import defaultdict
    buckets=defaultdict(list)
    for it in items:
        reading=it.get("conj_reading") or it.get("conjugated","")
        buckets[_sim_key(reading)].append(it)
    return {k:v for k,v in buckets.items()
            if len({i["word"] for i in v})>=2}

class SimilaritySession:
    """
    听音 → 从 N 个发音相近的词中选出正确的那个
    答案确认（confirm）之后才推进进度，方便展示对/错反馈。
    """
    def __init__(self, items, choices_n=4):
        self.all_items=items
        self.choices_n=choices_n
        self.pool=build_similarity_pool(items)
    ...
    def _make_questions(self):
        qs=[]
        for key,group in self.pool.items():
            for target in group:
                distractors=[it for it in group if it["word"]!=target["word"]]
                extra=[it for it in self.all_items
                       if it["word"]!=target["word"] and it not in distractors]
                random.shuffle(extra)
                distractors=distractors+extra
                chosen_dist=random.sample(distractors,
                                          min(self.choices_n-1,len(distractors)))
                choices=chosen_dist+[target]
                random.shuffle(choices)
                qs.append({"target":target,"choices":choices,
                           "correct_idx":choices.index(target),"key":key})
        return qs
...
def screen_similarity_session():
    ...
    audio_key=f"sim::{target['word']}::{target['form_id']}::{st.session_state.speed}"
    if st.session_state.last_audio_key!=audio_key:
        ...
        st.session_state.sim_chosen=None   # 新题重置选择
    ...
    st.markdown(
        ...
        f"<span class='form-tag'>{target['form_label']}</span>"
        f"&nbsp; 听音频，选出正确的单词</div>",
        unsafe_allow_html=True)
    ...
    chosen=st.session_state.get("sim_chosen")   # None = 未答
    ...
    if chosen is None:
        ...
        tgt_ch=q["choices"][q["correct_idx"]]
        if is_correct:
            st.success(f"✅ 正确！ **{tgt_ch['word']}**（{tgt_ch.get('conj_reading','')}）")
        else:
            chosen_word=q["choices"][chosen]["word"]
            st.error(f"❌ 你选了 **{chosen_word}**，正确答案是 **{tgt_ch['word']}**"
                     f"（{tgt_ch.get('conj_reading','') or tgt_ch.get('conjugated','')}）")
