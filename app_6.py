...
def render_audio(data: bytes, word: str = "", autoplay: bool = False) -> None:
    """
    用 st.components.v1.html（独立 iframe）渲染音频。
    关键：JavaScript 显式调用 audio.load() 强制浏览器加载新音频，
    解决 st.markdown/innerHTML 方案下浏览器复用旧缓冲音频的问题。
    """
    if not data:
...
<script>
  ...
  audio.load();   /* 强制浏览器重新读取 source，不沿用旧缓冲 */
  {play_js}
</script>
</body>
</html>"""
...
def _init():
    defaults = {
        ...
        'show_word': False,   # toggle 初始默认值（首次渲染前用）
        ...
        'last_audio_word': '',    # 上次加载音频的词，用于检测词是否切换
        # 文件上传去重
        'last_file_id':     '',
...
def _panel_study():
    ...
    if st.button("🎧 开始练习", type="primary",
        ...
        st.session_state.pop('show_word', None)   # 重置 toggle
        st.session_state.last_audio_word = ''   # 重置，触发首词音频加载
...
def screen_session():
    ...
    # ── 核心修复：在渲染任何 UI 之前，检测词是否切换并加载音频 ──────
    # 原来在 do_rate 里调用 _load_audio() 再 st.rerun() 的方式
    # 在 Streamlit 的执行流里不可靠。改为在每次 rerun 最顶部自检：
    # 只要当前词和上次加载的词不同，就重新获取音频。
    ...
    # ── 单词显示 ──────────────────────────────────────
    # 用 key='show_word' 让 Streamlit 直接管理 toggle 状态，
    # do_rate/skip 时直接写 session_state.show_word=False 即可，
    # 不需要 value= 同步，彻底解决"需要点两次"的问题。
    ...
    if st.session_state.cur_audio:
        # st.audio 在主页面上下文渲染，能继承用户点击手势，
        # 浏览器自动播放策略不会拦截，彻底解决 iframe 里 autoplay 失效的问题。
    ...
    def do_rate(lv):
        st.session_state.pop('show_word', None)  # 重置 key 绑定的 toggle
        ...
        # 不需要手动调用 _load_audio：
        # rerun 后 screen_session 顶部会检测词变化并自动加载音频
        st.rerun()
