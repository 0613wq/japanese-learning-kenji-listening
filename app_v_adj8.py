...
def screen_main():
    ...
    # 相似音顺序练习：按 (form_id, 变形读音前缀) 聚类排序后，以相同界面练习
    sim_items=sort_by_similarity(
        [it for it in cs.build_practice_items(ws,ev,ea,0) if it.get("conj_reading")])
    sim_disabled=(len(sim_items)<2)
    sim_tip=(f"🔀 相似音顺序练习（{len(sim_items)} 条，按读音聚类排序）"
             if not sim_disabled else "🔀 相似音练习（需要有变形读音的词）")
    if st.button(sim_tip,disabled=sim_disabled,use_container_width=True):
        st.session_state.voice=VOICES[voice_name]
        st.session_state.session=PracticeSession(sim_items,ordered=True)
        st.session_state.show_answer=False
        st.session_state.last_audio_key=""
        st.session_state.phase="session"; st.rerun()
...
# ═══════════════════════════════════════════════════
def sort_by_similarity(items, prefix_len=3):
    """
    按 (form_id, 变形读音前 prefix_len 字, 原词) 排序。
    同一变形类型下读音相近的词将相邻出现，便于耳朵对比辨别。
    """
    def _key(it):
        r=(it.get("conj_reading") or it.get("conjugated","")).strip()
        prefix=r[:prefix_len] if len(r)>=prefix_len else r
        return (it.get("form_id",""), prefix, it.get("word",""))
