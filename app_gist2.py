...
def sort_words_by_similarity(words, prefix_len=2):
    """
    按读音前 prefix_len 字排序，发音相近的词相邻出现。
    举例（prefix_len=2）：
      聞く（きく）/ 効く（きく）→ 排在一起
      書く（かく）/ 掛く（かく）→ 排在一起
    """
...
def _panel_study():
    ...
    # 相似音顺序练习：按读音前缀聚类排序，以相同界面练习
    sim_words_all = [w for w in ws]   # 全部筛选词，无读音的用词本身
    sim_disabled  = (len(sim_words_all) < 2)
    sim_label = (f"🔀 相似音顺序练习（{len(sim_words_all)} 词，按读音聚类排序）"
                 if not sim_disabled else "🔀 相似音练习（筛选词不足）")
    if st.button(sim_label, disabled=sim_disabled, use_container_width=True):
        st.session_state.voice       = VOICES[voice_name]
        sorted_words = sort_words_by_similarity(sim_words_all)
        # 全部放入同一组，保持排序顺序
        for w in sorted_words:
            w = w   # 不改动原词典，group 原样保留
        st.session_state.session = SessionManager(sorted_words, ordered=True)
        st.session_state.pop('show_word', None)
        st.session_state.last_audio_word = ''
        st.session_state.phase = 'session'
        st.rerun()
