...
def _panel_study():
    ...
    # ── 每组词数 ──────────────────────────────────────
    st.caption("**每组词数**（练习时按此大小分批，超过阈值的卡词会跨组继续出现）")
    batch_size = st.number_input(
        "每组词数", min_value=5, max_value=200,
        value=st.session_state.get("batch_size", 20), step=5,
        label_visibility="collapsed", key="batch_size_input",
    )
    ...
    if ws:
        import math
        n_batches = math.ceil(len(ws) / bs)
        st.info(f"🎯 **{len(ws)}** 词  ÷  每组 **{bs}**  =  **{n_batches}** 组")
    else:
    ...
    if st.button("🎧 开始练习", type="primary",
        ...
        batched = [{**w, 'group': (i // bs) + 1} for i, w in enumerate(ws)]
        st.session_state.session = SessionManager(batched)
    ...
    # ── 相似音顺序练习：先按读音排序，再分组 ─────────
    sim_disabled = (len(ws) < 2)
    sim_label = (f"🔀 相似音顺序练习（先按读音排序，再分 {math.ceil(len(ws)/bs) if ws else 0} 组）"
                 if not sim_disabled else "🔀 相似音练习（筛选词不足）")
    if st.button(sim_label, disabled=sim_disabled, use_container_width=True):
        st.session_state.voice = VOICES[voice_name]
        sorted_words = sort_words_by_similarity(ws)
        batched = [{**w, 'group': (i // bs) + 1} for i, w in enumerate(sorted_words)]
        st.session_state.session = SessionManager(batched, ordered=True)
        st.session_state.pop('show_word', None)
        st.session_state.last_audio_word = ''
        st.session_state.phase = 'session'
        st.rerun()
