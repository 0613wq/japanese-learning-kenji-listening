...
def screen_session():
    ...
    # ── 显词开关（必须在词语显示之前渲染，才能在同一次 rerun 里生效）──
    show_now = st.toggle("👁 显示单词", value=st.session_state.show_word)
    st.session_state.show_word = show_now   # 同步回 session_state
    ...
    if st.session_state.cur_audio:
        render_audio(st.session_state.cur_audio, word=word, autoplay=autoplay)
    else:
